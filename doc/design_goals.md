...
We are well aware of the IDGA benefits. We have actually talked with Bondtech several years ago when the Bondtech Prusa extruder was introduced and more recently for the development of the IDGA. We also thought to design our own gears as we see improvements but making a custom motor was already a big step. When we released the BearExxa V2 beta, the IDGA product was unfortunately not available and so it's not supported yet. We will make the final version compatible with BMG IDGA or OEM IDGA (we don't know which one is best yet). However, we can guarantee that even without the IDGA the print quality is excellent and 602 pattern (moiré, wood grain. inconsistency) has been drastically reduced.
